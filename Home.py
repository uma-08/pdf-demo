__version__ = "0.0.1"
app_name = "Incribo AuroraAI Demo Model"
...
def ui_info():
	...
	st.markdown("""
		Note:
		Please be aware that this is only a demo model
...
def ui_question():
	...
	disabled = False
	st.text_area('question', key='question', height=100, placeholder='Ask questions related to the PDF file', help='', label_visibility="collapsed", disabled=disabled)
...
def b_ask():
	...
	if c1.button('Generate answer', disabled=disabled, type='primary', use_container_width=True):
		question = ss.get('question','')
		temperature = ss.get('temperature', 0.9)
...
def b_save():
	...
	if st.button('save encrypted index in Incribo database', disabled=disabled, help=help):
...
def b_delete():
	...
	# TODO: confirm delete
	if st.button('delete from Incribo database', disabled=not db or not name):
